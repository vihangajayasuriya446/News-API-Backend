...
export class ArticlesService {
  ...
  async create(createArticleDto: CreateArticleDto, author: User): Promise<ArticleResponseDto> {
    ...
    Object.assign(article, {
      ...
      published_at: createArticleDto.is_published ? new Date() : undefined
    });
  ...
  async update(id: number, updateArticleDto: UpdateArticleDto): Promise<ArticleResponseDto> {
    ...
    if (updateArticleDto.title) {
      updatedData.slug = this.generateSlug(updateArticleDto.title);
    }

    if (updateArticleDto.is_published !== undefined) {
      updatedData.published_at = updateArticleDto.is_published 
        ? (article.is_published ? article.published_at : new Date())
        : undefined;
    }

    const updatedArticle = await this.articleRepository.save(updatedData);
